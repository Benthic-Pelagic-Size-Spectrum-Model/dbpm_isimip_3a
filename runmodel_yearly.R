if(!require("stringr", quietly = TRUE, warn.conflicts = FALSE)) install.packages("shinydashboard")
...
rungridsep_ssp <- function(igrid
  ...
  if(stringr::str_detect(output_historical_location,"ISIMIP3a")){ # if using ISIMIP3a
    curr_grid_output_h <- list.files(path = output_historical_location, 
                                     pattern = paste0("dbpm_output_all_", igrid, '_', protocol, '.rds'), 
                                     full.names = TRUE)
  } else {
    curr_grid_output_h <- list.files(path = output_historical_location, 
                                     pattern = paste0("dbpm_output_all_", igrid, '_', 'historical', '.rds'), 
                                     full.names = TRUE)
  }
